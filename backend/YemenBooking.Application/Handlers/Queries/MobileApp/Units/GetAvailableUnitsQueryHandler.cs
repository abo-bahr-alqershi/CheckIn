...
public class GetAvailableUnitsQueryHandler : IRequestHandler<GetAvailableUnitsQuery, ResultDto<AvailableUnitsResponse>>
{
        ...
        try
        {
            ...
            var availableUnits = new List<AvailableUnitDto>();
                ...
                foreach (var unit in eligibleUnits)
                {
                    ...
                    var isAvailable = !request.CheckIn.HasValue || !request.CheckOut.HasValue ? true : await IsUnitAvailable(unit.Id, request.CheckIn.Value, request.CheckOut.Value, cancellationToken);
                    ...
                    var (totalPrice, pricePerNight) =  !request.CheckIn.HasValue || !request.CheckOut.HasValue ? (0, 0) : await CalculateUnitPrice(unit, request.CheckIn.Value, request.CheckOut.Value, cancellationToken);
    ...
    private ResultDto<AvailableUnitsResponse> ValidateRequest(GetAvailableUnitsQuery request)
    {
        ...
        if (request.CheckIn.HasValue && request.CheckIn.Value.Date < DateTime.Today)
        {

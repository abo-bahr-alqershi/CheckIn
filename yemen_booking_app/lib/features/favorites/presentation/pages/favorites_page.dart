  ...
  List<dynamic> _filterAndSortFavorites(List<dynamic> favorites) {
    var filtered = favorites.where((favorite) {
      ...
      if (_searchQuery.isNotEmpty) {
        ...
        final title = favorite.property?.title?.toLowerCase() ?? '';
        final location = favorite.property?.location?.toLowerCase() ?? '';
      ...
      if (_selectedFilter != 'all') {
        final type = favorite.property?.type ?? '';
    ...
    switch (_sortBy) {
      ...
      case 'price_low':
        filtered.sort((a, b) => 
          (a.property?.price ?? 0).compareTo(b.property?.price ?? 0));
        break;
      case 'price_high':
        filtered.sort((a, b) => 
          (b.property?.price ?? 0).compareTo(a.property?.price ?? 0));
        break;
      case 'name':
        filtered.sort((a, b) => 
          (a.property?.title ?? '').compareTo(b.property?.title ?? ''));
        break;
    }
